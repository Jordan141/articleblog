...
router.get('/approve', isLoggedIn, async (req, res) => {
    ...
    try {
        ...
        return res.render('pages/approve', {title: 'Approve Articles', articles, currentUser: req.user, categories: CATEGORIES_LIST, isReviewing: true})
    } catch(err) {
...
//UPDATE Route
router.put('/:link', checkArticleOwnership, async (req, res) => {
    if(req.params.link === undefined) {
        req.flash('error', 'Oops! Something went wrong!')
